...
class HNetEmbeddingStore(BaseStore):
    ...
    def get_or_create(self, text: str):
        ...
        try:
            with self.conn.cursor() as cur:
                cur.execute(
                    "SELECT embedding FROM hnet_embedding WHERE text_hash  = %s",
                    (text_hash,),
                )
                row = cur.fetchone()
                if row:
        ...
        try:
            with self.conn.cursor() as cur:
                cur.execute(
                    """
                    INSERT INTO hnet_embedding (text, text_hash, embedding)
                    VALUES (%s, %s, %s)
    ...
    def backfill_prompt_embeddings(self):
        ...
        for prompt_id, prompt_text in rows:
            ...
            with self.conn.cursor() as cur:
                cur.execute(
                    "SELECT id FROM hnet_embedding WHERE text_hash = %s", (text_hash,)
                )
        ...
        try:
            ...
            with self.conn.cursor() as cur:
                cur.execute(
                    ...
                    FROM documents d
                    JOIN hnet_embedding e ON d.embedding_id = e.id
